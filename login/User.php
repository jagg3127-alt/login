...
class User extends Dbconfig {	
		...
		if(!empty($_POST["register"]) && $_POST["email"] !='') {
			...
			if($isUserExist) {
				...
				if($userSaved) {				
					...
					$msg = wordwrap($msg,70);
					$headers = "From: jhaught2007@gmail.com";
	...
	public function editAccount () {
		...
		$updateQuery = "UPDATE ".$this->userTable." 
			SET first_name = '".$_POST["firstname"]."', last_name = '".$_POST["lastname"]."', email = '".$_POST["email"]."', mobile = '".$_POST["mobile"]."' , designation = '".$_POST["designation"]."'
$updatePassword
			WHERE id ='".$_SESSION["userid"]."'";
		$isUpdated = mysqli_query($this->dbConnect, $updateQuery);	
		if($isUpdated) {
	...
	public function resetPassword(){
		...
		if($_POST['email'] == '') {
			...
			if($numRows) {			
				...
				$msg = wordwrap($msg,70);
				$headers = "From: mail.lunarwriting@gmail.com";
				if(mail($toEmail, $subject, $msg, $headers)) {
